...
const EditPromo = () => {
  ...
  const [descript, setDescript] = useState(data.description);
  const [expDate, setExpDate] = useState(new Date(data.expired_at));
  const [showPicker, setShowPicker] = useState(false);

  const onChangeDiscount = text => {
    if (parseInt(text) > 100) return;
    setDiscount(text);
    const newPrice = (parseInt(data.price) * parseInt(text)) / 100;
    if (parseInt(text) > 0)
      return setDiscPrice(parseInt(data.price) - newPrice);
    setDiscPrice('');
  };
  ...
  return (
    <ScrollView>
      <ModalMsg
        ...
        msg={`Are you sure want to delete ${data.prod_name} from promo list?`}
      ...
      <View style={styles.mainScreen}>
        <View style={{alignItems: 'center'}}>
          ...
          <Text style={styles.textName}>{data.prod_name}</Text>
